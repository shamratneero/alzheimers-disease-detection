...
OOF_PATH = Path(
    "results/baseline_model/strict_baseline_oof_predictions.csv"
)
...
y = df["STRICT_LABEL"].astype(int).to_numpy()
p = df["OOF_PROBABILITY"].astype(float).to_numpy()

print("=" * 72)
print("STRICT BASELINE OOF PROBABILITY + CALIBRATION AUDIT")
print("=" * 72)
...
for label, name in [(0, "NON-CONVERTER"), (1, "STRICT CONVERTER")]:
    x = p[y == label]
...
print(
    "\nImportant: this threshold is descriptive only. "
    "It should not be treated as a deployment or clinical threshold."
)