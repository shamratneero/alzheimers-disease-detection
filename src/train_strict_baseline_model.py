...
# Strict cohort only:
# STRICT_LABEL = 1 -> confirmed converter
# STRICT_LABEL = 0 -> non-converter
# NaN -> excluded from strict sensitivity analysis
df = df[df[TARGET].notna()].copy()

# Complete-case predictors
df = df.dropna(
    subset=PREDICTORS + [TARGET]
).copy()

print("=" * 72)
print("STRICT-LABEL BASELINE MODEL")
print("=" * 72)

print("\nRows after strict-label + complete-case filtering:", len(df))
print("Unique RID:", df["RID"].nunique())

y = df[TARGET].astype(int).to_numpy()
X = df[PREDICTORS].copy()

print("Strict converters:", int(y.sum()))
print("Non-converters:", int((y == 0).sum()))

# Expected counts
assert int(y.sum()) == 152, \
    f"Expected 152 strict converters, got {int(y.sum())}"

# We had 822 complete-case negatives in the primary model
assert int((y == 0).sum()) == 822, \
    f"Expected 822 complete-case negatives, got {int((y == 0).sum())}"
...
pooled_pr_auc = average_precision_score(y, oof_prob)
pooled_brier = brier_score_loss(y, oof_prob)
pooled_bacc = balanced_accuracy_score(y, oof_pred)
...
print("\n" + "=" * 72)
print("STRICT POOLED OUT-OF-FOLD PERFORMANCE")
print("=" * 72)
...
oof_path = (
    OUTPUT_DIR /
    "strict_baseline_oof_predictions.csv"
)
...
fold_path = (
    OUTPUT_DIR /
    "strict_baseline_fold_metrics.csv"
)
...
summary = {
    ...
    "pooled_sensitivity": float(pooled_sensitivity),
    "pooled_specificity": float(pooled_specificity),
}

summary_path = (
    OUTPUT_DIR /
    "strict_baseline_summary.json"
)
